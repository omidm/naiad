...
def run_experiment(worker_ips, worker_p_dnss):
  ...
  for idx in range(0, config.WORKER_INSTANCE_NUM):
    ...
    for w in range(0, config.WORKER_PER_INSTANCE):
      ...
      run_worker(worker_num, ip, pid, host_list);


def run_worker(worker_num, worker_ip, pid, host_list):
...
def terminate_experiment(worker_ips):
...
def collect_output_data(worker_ips):
  ...
  for ip in worker_ips:
    subprocess.Popen(['scp', '-r', '-i', config.PRIVATE_KEY,
        '-o', 'UserKnownHostsFile=/dev/null',
        '-o', 'StrictHostKeyChecking=no',
        'ubuntu@' + ip + ':' + config.EC2_NAIAD_ROOT +
